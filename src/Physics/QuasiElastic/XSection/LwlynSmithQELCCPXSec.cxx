   ...
   const Interaction * interaction, KinePhaseSpace_t kps) const
{
  ...
  double xsec = /*Fp*/ Gfactor * (A + sign*B*s_u/M2 + C*s_u*s_u/M4);
...
void LwlynSmithQELCCPXSec::LoadConfig(void)
{
  ...
  double Ev = 1.0; // neutrino energy
  //int pdgv = 12; // electron neutrino PDG code
  int pdgv = 14; // muon neutrino PDG code
  int pdgtgt = 1000000010; // free neutron nucleus PDG code (check)
  int pdgnuc = 2112; // code for neutron, hit nucleon
  Interaction * interaction = Interaction::QELCC(pdgtgt, pdgnuc, pdgv, Ev);
  ...
  for (int i=0;i<11;i++) {
    ...
    LOG("some_identifying_string",pWARN)
     << "Ev=" <<Ev
     << "Q2=" <<Q2
     <<" : xsec(Ev,Q2)= " <<this->XSec( interaction, kps);
  }
  delete interaction;
}
